<template>
  ...
  >
    <div
      ...
      v-bind:key="char.id"
      class="flip-card-front flex justify-center gap-1"
    >
      <div
    ...
    <div  v-for="(char, index) in row.letters"
        v-bind:key="char.id" class="flip-card-back  justify-center gap-1">
      <div
       
        class=" items-center justify-center border-2 inputF border-gray-400 w-16 h-16 text-3xl font-bold text-center"
        v-bind:id="'letter-' + rowIndex + '-' + ++index"
        v-bind:class="{
          darkModeRows: darkMode,
          active1: char.letter.length > 0,
